  ...
  if (rxp.test(message)) {
    try {
      ...
      if (isRepeating) {
        ...
        const timePatterns = [
          /[ов]\s?(\d{1,2})(?::(\d{2}))?\s*(?:год|годин)/i, // "о 10 годині"
          /[ов]\s?(\d{1,2})(?::(\d{2}))?/i, // "о 15:30"
          /(\d{1,2}):(\d{2})/i, // "15:30"
          /(\d{1,2})\s*год(?:ин[іуа])?/i, // "10 годині", "15 год"
          /(\d{1,2})\s*(?:год(?:ин[іуа])?)??\s*ранку/i, // "10 ранку"
          /(\d{1,2})\s*(?:год(?:ин[іуа])?)??\s*вечора/i, // "8 вечора"
          /(\d{1,2})\s*(?:год(?:ин[іуа])?)??\s*дня/i, // "2 дня"
        ]
